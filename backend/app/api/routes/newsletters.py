...
@router.post(
    "/cleanup",
    response_model=NewsletterCleanupResponse,
    status_code=status.HTTP_200_OK,
    summary="Clean up old newsletters",
    description="Delete newsletters older than 7 days that are not in any bookmark list"
)
async def cleanup_newsletters(
    ...
    """
    Clean up old newsletters for the authenticated user.

    Deletes newsletters that meet ALL of the following criteria:
    - Belong to the authenticated user
    - Received date is older than 7 days
    - NOT in any bookmark list

    This helps keep the user's newsletter database clean while preserving
    bookmarked newsletters regardless of age.
    ...
    Returns:
        NewsletterCleanupResponse: Cleanup operation result with deleted count

    Raises:
        HTTPException 500: If cleanup operation fails
    """
    logger.info(f"Starting newsletter cleanup for user {current_user.id}")
    ...
    try:
        ...
        logger.info(
            f"Cleanup cutoff date: {cutoff_date.isoformat()} for user {current_user.id}"
        )

        # Find newsletters to delete using a subquery approach
        # Step 1: Find newsletter IDs that are in ANY bookmark list
        bookmarked_newsletters_subquery = (
            select(NewsletterBookmark.newsletter_id)
            .distinct()
        ).subquery()

        # Step 2: Build query to find newsletters to delete
        # - user_id = current_user.id
        # - received_date < cutoff_date
        # - id NOT IN bookmarked_newsletters
        newsletters_to_delete_query = select(Newsletter.id).where(
            and_(
                Newsletter.user_id == current_user.id,
                Newsletter.received_date < cutoff_date,
                Newsletter.id.notin_(bookmarked_newsletters_subquery)
            )
        )

        # Execute query to get count before deletion
        result = await db.execute(newsletters_to_delete_query)
        newsletter_ids = [row[0] for row in result.all()]
        count_to_delete = len(newsletter_ids)

        logger.info(
            f"Found {count_to_delete} newsletters to delete for user {current_user.id}",
            extra={
                "user_id": current_user.id,
        ...
        # Perform the delete operation if there are newsletters to delete
        deleted_count = 0
        if count_to_delete > 0:
            # Use delete statement with the same conditions
            delete_stmt = delete(Newsletter).where(
                and_(
                    Newsletter.user_id == current_user.id,
                    Newsletter.received_date < cutoff_date,
                    Newsletter.id.notin_(bookmarked_newsletters_subquery)
            ...
            logger.info(
                f"Successfully deleted {deleted_count} newsletters for user {current_user.id}",
                extra={
                    "user_id": current_user.id,
            ...
            logger.info(
                f"No newsletters to delete for user {current_user.id}",
        ...
        return NewsletterCleanupResponse(
            status="success",
            deleted_count=deleted_count,
            timestamp=timestamp
        )
        ...
        logger.error(
            f"Error during newsletter cleanup for user {current_user.id}: {str(e)}",
            exc_info=True,
            extra={
                "user_id": current_user.id,
                "error_type": type(e).__name__,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error cleaning up newsletters: {str(e)}"
        )
